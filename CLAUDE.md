...
- 労務管理（実装済み）: 職員名簿・シフト管理・勤怠管理・有給休暇管理・時間外／従業員アプリ
- 文書管理（実装済み）: 様式・規則をGoogleドライブの共有リンクで登録し、従業員が種別ごとに閲覧。
  事務局 `/labor/documents` で登録・編集・削除、従業員 `/me/documents` で閲覧。閲覧は認証済みなら誰でも可
  （GAS AUTHED_ACTIONS）、登録/削除は事務局のみ。DocumentItem(type:form/rule/other, title, url)。GAS: documents シート。
- 会計管理（予定）
- 備品・施設管理（予定）
