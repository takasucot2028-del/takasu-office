...
- **確定モード**: 各セルをクリックでポップアップが開き、区分を複数トグルできる（1日に午前＋午後など複数可）。希望の○×を背景色でヒント表示。確定は ConfirmedShift（staffId+date+location+patternId、1セルにつき複数レコード）。勤務日数は「区分が1つ以上ある日」を1日と数え、実働時間は全区分の合計。下部に日別人数、右に職員別の勤務日数・実働時間。
