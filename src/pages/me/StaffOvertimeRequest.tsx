...
export default function StaffOvertimeRequest() {
  ...
  const add = async (e: React.FormEvent) => {
    ...
    if (!hrs || hrs <= 0) { setError('予定時間を入力してください'); return; }
    setSaving(true); setError(''); setMessage('');
    try {
      await addMyOvertime({ date, appliedHours: hrs, reason, kind: overtimeKindOf(staff, date) });
      setMessage('時間外を申請しました。事務局の承認をお待ちください。');
      setReason(''); setHours('1');
      setVersion(v => v + 1);
    } catch (err) {
    ...
    <PageContainer title="時間外・休日勤務の申請">
      ...
      <Card className="mb-4">
        ...
        <form onSubmit={add} className="grid sm:grid-cols-4 gap-3 items-end">
          ...
          <Field label="予定時間（h）">
            <Input type="number" min={0} step={0.5} value={hours} onChange={e => setHours(e.target.value)} required />
          </Field>
          <Field label="事由">
        ...
        <p className="text-xs text-gray-400">実績時間・手当/代休は、勤務後に事務局が勤怠をもとに確定します。</p>
      </Card>
