                  ...
                  return (
                    <th key={d} title={closedDayName(d) || undefined}
                      className={`border border-gray-400 px-0 py-1 text-center ${
                        isSpecialHoliday(d) || wd === 0 ? 'bg-red-100' : wd === 6 ? 'bg-blue-100' : 'bg-gray-100'}`}>
                      <div>{Number(d.slice(8))}</div>
                      <div className="text-[10px]">{isSpecialHoliday(d) ? '休' : WEEKDAY_LABELS[wd]}</div>
                    </th>
                  );
                })}
                      ...
                      return (
                        <td key={date}
                          className={`border border-gray-400 text-center px-0 py-1 ${
                            ng ? 'bg-red-100'
                              : isSpecialHoliday(date) || wd === 0 ? 'bg-red-50'
                                : wd === 6 ? 'bg-blue-50' : ''}`}>
                          {ids.length
