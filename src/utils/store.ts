...
function seedDemo() {
  ...
  if (seeded) {
    ...
    localStorage.setItem(KEY_SEEDED, '6');
    return;
  }
  const now = new Date().toISOString();
  const demo: Staff[] = [
    {
      ...
      hourlyWage: 1500, monthlyHourLimit: 0, childNursingChildren: 2, note: '', createdAt: now, updatedAt: now,
    },
    {
      ...
      hourlyWage: 1100, monthlyHourLimit: 88, childNursingChildren: 0, note: '週4日勤務', createdAt: now, updatedAt: now,
    },
    {
      ...
      hourlyWage: 1200, monthlyHourLimit: 0, childNursingChildren: 0, note: '', createdAt: now, updatedAt: now,
    },
  ];
  save(KEY_STAFF, demo);
